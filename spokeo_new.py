...
def read_data():
    ...
    f.close()
    agents = [i.strip('\n') for i in uas]
    with open('/home/spider/pii/spokeo/config/label_spokeo_tormarket.txt', 'r') as f1:
        records = f1.readlines()
    f1.close()
    piis = [i.strip('\n') for i in records]
    len_pii = len(piis)
    piis = piis[int(len_pii * 2 / 5):int(len_pii * 3 / 5)]
    print('----Read End----')
...
def get_mylife(url, name, city):
    if city == '':
        city = 'None'
    with open('/home/spider/pii/spokeo/config/saved_url_beenverfied', 'a') as f:
...
def get_url(query, name, city):
    ...
    bing_url = 'https://www.bing.com/search?q=' + str(query) + '%20beenverified' + '&qs=n&form=QBRE'
    print(bing_url)
...
if __name__ == "__main__":
    ...
    tor_reset()
    time = 0
    with open('/home/spider/pii/spokeo/config/stop_point_3', 'r') as f:
        temp = f.readline()
    ...
    for pii in piis:
        ...
        if time < 0:
            continue
        if time % 100 == 0:
            with open('/home/spider/pii/spokeo/config/stop_point_3', 'w') as f:
                f.write(str(time) + '\n')
            f.close()
